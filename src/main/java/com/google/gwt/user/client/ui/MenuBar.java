...
/* yan0kom
 * fork from 2.8.2 to support enable/disable and to show submenu above menubar
 */
...
public class MenuBar extends Widget implements PopupListener, HasAnimation,
  ...
  private final class MenuPopup extends DecoratedPopupPanel {
    ...
    public void positionAbove(MenuItem target) {
        int top = MenuBar.this.getAbsoluteTop() - target.getSubMenu().getOffsetHeight() - 12;
        int left = towardsEast ? leftOf(target) : rightOf(target) - getOffsetWidth();
        setPositionInClient(left, top);
    }
  ...
  private void openPopup(final MenuItem item) {
    ...
    popup.setPopupPositionAndShow(new PositionCallback() {
      ...
      public void setPosition(int offsetWidth, int offsetHeight) {
        ...
        }else if (submenuAbove) { 
        	popup.positionAbove(item);
        }else {
  ...
  public boolean isSubmenuAbove() {
	return submenuAbove;
  }

  public void setSubmenuAbove(boolean submenuAbove) {
	this.submenuAbove = submenuAbove;
  }
